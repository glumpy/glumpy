...
fragment = """
    ...
    void main()
    {
        float x = v_texcoord.x * u_viewport.x; // - u_translate.x;
        float y = v_texcoord.y * u_viewport.y; // - u_translate.y;

//        vec2 P = v_texcoord - vec2(0.5,0.5);
        vec2 P = v_texcoord - u_translate/u_viewport;

        float rho = length(P) / sqrt(2.0);
        float theta = atan(P.y,P.x);

        float Tx = texture2D(u_grid, vec2(rho,0.5)).x;
        float tx = texture2D(u_grid, vec2(rho,0.5)).z;

        float Mx = abs(rho * u_viewport.x - Tx);
        float mx = abs(rho * u_viewport.x - tx);

        float t = (theta + M_PI)/(2*M_PI);
        float Ty = 2*M_PI*(texture2D(u_grid, vec2(t,0.5)).y/1000.0 - 1.0);
        float ty = 2*M_PI*(texture2D(u_grid, vec2(t,0.5)).w/1000.0 - 1.0);

        float My = abs(sin(theta-Ty) * (rho * u_viewport.x));
        float my = abs(sin(theta-ty) * (rho * u_viewport.x));
...
def update_grid(w, h):
    ...
    I1 = np.arange(0, w, t1)
    I2 = np.arange(0, w, t2)
    I3 = np.linspace(0,1,12)*1000
    I4 = np.linspace(0,1,111)*1000
...
program['u_translate'] = window.width/2.0,window.height/2.0
program['u_scale'] = 1.0
...
translate = window.width/2.0,window.height/2.0
scale = 1
