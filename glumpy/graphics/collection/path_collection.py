...
def PathCollection(mode="agg-fast", *args, **kwargs):
    """
    mode: string
      - "raw"      (speed: fastest, size: small,   output: ugly, no dash, no thickness)
      - "agg-fast" (speed: medium,  size: medium   output: nice, flaws, no dash)
      - "agg"      (speed: slow,    size: big,     output: perfect, no dash)
      - "agg-dash" (speed: slowest, size: biggest  output: perfect)
    """
